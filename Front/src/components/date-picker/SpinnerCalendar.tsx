...
const SpinnerCalendar: React.FC<SpinnerCalendarProps> = ({
  ...
  const move = (behavior: "smooth" | "instant" = 'smooth') => {
    document.getElementById(`year_${currentYear}`)?.scrollIntoView({ behavior, block: 'center' });
    document.getElementById(`month_${currentMonth + 1}`)?.scrollIntoView({ behavior, block: 'center' });
    document.getElementById(`date_${currentDate}`)?.scrollIntoView({ behavior, block: 'center' });
  }
  useEffect(() => {
  ...
  useEffect(() => {
    const columns = [{
      min: 1900, max: 3000, id: 'year', onWheel: (delta: number) => {
        if (delta > 0) {
      ...
      min: 1, max: 12, id: 'month', onWheel: (delta: number) => {
        if (delta > 0) {
    ...
    columns.push({
      min: 1, max: date.getDate(), id: 'date', onWheel: (delta: number) => {
        if (delta > 0) {
  ...
  return (
    <div className={styles.spinnerCalendarContainer}>
      ...
      <div className={styles.spinnerColumnsWrapper}>
        ...
        {columns.map(column => <div id={column.id} key={column.id} className={styles.spinnerColumnLine} style={{ overflow: preventScroll ? 'hidden' : 'scroll' }}>
          {Array.from({ length: column.max - column.min }).map((_, index) =>
            <p id={`${column.id}_${index + column.min}`} key={`${column.id}_${index + column.min}`} className={styles.spinnerColumn} onWheel={(e) => column.onWheel(e.deltaY)} onTouchEnd={(e) => {
              setPreventScroll(true)
              setTimeout(() => {
                setPreventScroll(false)
                const scroll = document.getElementById(column.id)
                column?.onTouchEnd(column.min + Math.round((scroll?.scrollTop || 0) / 30))
              }, 300)
            }}>
              {index + column.min}
            </p>)}
