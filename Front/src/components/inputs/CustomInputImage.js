...
import style from "./CustomInputImage.module.css";
import EasyCrop from "./EasyCrop";
import Loader from "./Loader";

const CustomInputImage = forwardRef(
  (
    {
      values = [],
      maxSizeMb,
      ...
      <div style={{ position: "relative" }}>
        ...
        <VerticalFlex gap={16}>
          ...
          <FlexChild
            ...
            onClick={() => {
              if (files.length > 0 || userThumbnail) {
                NiceModal.show("confirm", {
                  message: "wannaDeleteThumbnail",
            ...
            cursor={"pointer"}
          >
            <HorizontalFlex gap={7} justifyContent={"center"}>
              <FlexChild width={"fit-content"}>
                <Icon
                  name={
                    files.length > 0 || userThumbnail ? "trash" : "cameraRedBig"
                  }
                  width={20}
                />
              </FlexChild>
              <FlexChild width={"fit-content"}>
                <P size={15} color={"#8B8B8B"}>
                  {files.length > 0 || userThumbnail
                    ? "delete"
                    : "registerPhoto"}
                </P>
              </FlexChild>
          ...
          {/* <FlexChild>
            <HorizontalFlex justifyContent={"flex-start"} gap={10}>
              <FlexChild width={"max-content"}>
                ...
                >
                  <P size={16} textAlign="center" color={"var(--main-color)"}>
                    선택 삭제
                  </P>
                </Div>
              </FlexChild>
