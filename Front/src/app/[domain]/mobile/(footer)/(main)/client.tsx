    ...
    >
      <HorizontalFlex fontWeight={700}>
        ...
        <FlexChild width={"max-content"} gap={17}>
          <Image src={"/resources/icons/search.png"} size={16} />
          <Div position="relative">
            ...
            <Image src={"/resources/icons/cart.png"} size={16} />
          </Div>
        </FlexChild>
      </HorizontalFlex>
    </FlexChild>
  ...
  const { banners } = useData(
    ...
    (condition) =>
      Array.from({ length: 10 }).map((_, index) => ({
        id: String(index),
        url: "https://i.imgur.com/shaWEUs.png",
...
export function NewProducts({ initProducts }: { initProducts: Pageable }) {
  ...
  const { newProducts } = useData(
    "newProducts",
    { store_id: storeData?.id, pageSize: 10 },
...
export function HotProducts({ initProducts }: { initProducts: Pageable }) {
  const { storeData } = useStore();
  const [swiperInstance, setSwiperInstance] = useState<any>(null);
...
export function WeekProducts({ initProducts }: { initProducts: Pageable }) {
  const { storeData } = useStore();
  const [swiperInstance, setSwiperInstance] = useState<any>(null);
  const { weekProducts } = useData(
    "weekProducts",
    { store_id: storeData?.id, pageSize: 20 },
    (condition) => requester.getProducts(condition),
    {
      fallbackData: initProducts,
      revalidateOnMount: true,
        ...
        <SwiperSlide key={`week_${index}`}>
          <VerticalFlex gap={12}>
            {productList.map((product, idx) =>
              product ? (
                <FlexChild key={`best_${product.id}_${index * 5 + idx}`}>
...
export function ProductList({ initPorudcts }: { initPorudcts: Pageable }) {
  const list = [
    {
      label: "추천순",
      value: {},
  ...
  const { productList, page, maxPage, isLoading, Load } = useInfiniteData(
    "productList",
    (page) => {
      return {
        pageSize: 10,
